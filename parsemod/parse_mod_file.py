...
def generate_neuroml2_for_mod_file(mod_file, nmlfilepath):
    ...
    for line in blocks["NEURON"]:
        if line.startswith("SUFFIX"):
            info["id"] = line[7:].strip().split()[0]
        if line.startswith("USEION") and "WRITE" in line:
            info["species"] = line.split()[1]

    for line in blocks["BREAKPOINT"]:
        if line.startswith("g"+info["species"]):
            inst = line.split('*')
        else:
            continue
    ...
    for s in blocks["STATE"]:
        ...
        gate["id"] = s
        gate["type"] = "???"
        gate["instances"] = instance_count[s]
        gates.append(gate)
    ...
    for key, value in blocks.items():
        if key in ["PARAMETER"]:
            ...
            chan_file.write("\n###  " + key+" :\n")
            for par in value:
                if par.strip(" ").startswith(":"):
                    continue
                else:
                    chan_file.write(par+"\n")
    chan_file.write("### DELETE THE ABOVE TEXTS AFTER ADDING THE GATE INFO\n\n\n")
